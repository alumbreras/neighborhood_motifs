...
\begin{definition}
Given a tree graph $G$, the \textit{structure-based neighbourhood} of radius $r$ of post $i$, denoted as $\mathcal{N}_i(r)$, is the induced graph composed of all the vertices that are at distance equal or less than $r$ from post $i$.
\end{definition}
% pros and cons
...
Some of the anlysis in the next section requires that, if two type of neighbourhoods are isomorphics, they have the same label. Thus, we merge the dictionaries to obtain a global dictionary of neighbours. Figure XXX shows some frequent neighbourhoods that will be discussed later.
...
\includegraphics[width=0.5\textwidth]{confusion_time_order_zoom}%
\includegraphics[width=0.5\textwidth]{confusion_struct_time_zoom}
